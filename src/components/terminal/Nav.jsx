...
function Nav() {
    ...
    const { align, icons, text, effects, url, container } = navStyles;

    return (
        <nav className={container}>
            <a className={`${align} ${icons} ${effects}`}><MdAddCircleOutline /></a>
            <a className={`${align} ${icons} ${effects} mb-2`}><MdOutlineCancel /></a>
            <a href={url} target="_blank" rel="noopener noreferrer" className={`${align} ${effects} ${text} text-purple-500`}>new</a>
            <a className={`${align} ${effects} ${text} mb-2`}>sort</a>
        </nav>
    )
}
